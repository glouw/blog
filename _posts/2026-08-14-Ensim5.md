...
One can model a piston flow lane from plenum intake to exhaust as 9 chambers
(atmosphere, plenum, throttle, runner, piston, runner, exhaust, exhaust, atmosphere) with 8 nozzles,
mapping perfectly to floating point AVX256 or doubling point AVX512. The latter is required for stability, but sharing
at least the nozzle velocity equation with an `N` of `8`:
...
This allows for a real time engine to compute an entire piston's chamber flow speed from intake to exhaust with a couple cache line reads
...
for eight pistons in parallel, in this inline-8 configuration:
...
While single threaded performance is _excellent_ on even a _2019 T490_, pipe audio fidelity is reduced due
